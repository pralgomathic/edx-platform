...
($, ui, gettext, Backbone, NotificationView, PromptView, ModuleEditView, ModuleModel) ->
  class UnitEditView extends Backbone.View
    ...
    createComponent: (event, data, message) =>
      event.preventDefault()
    ...
    saveNewComponent: (event) =>
      editor = @createComponent(event, $(event.currentTarget).data(), "Adding")
      @closeNewComponent(event)
    ...
    duplicateComponent: (event) =>
      ...
      editor = @createComponent(event, {duplicate_source_locator: source_locator}, "Duplicating")
